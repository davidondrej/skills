...
description: Commit and push agent-skill changes to the user's private skills GitHub repo (your-private-skills, rooted at ~/.agents). Use after creating or updating any skill, when the user says "push the skill", "push skills to github", "save the skill to my repo", or "update the skills repo". Handles staging, committing, pushing, and cleaning up the cmux pane used to do it.
---

# Push Skills to GitHub

For committing any skill change to the user's private skills repo: **`your-private-skills`**, git root **`~/.agents`** (this is also the canonical skill folder; `.claude` and `.pi/agent/skills` symlink to `~/.agents/skills`). Pushes here auto-publish a sanitized public mirror — never push directly to that public repo.
