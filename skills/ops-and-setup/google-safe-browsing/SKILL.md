...
`trademark-tool-example.com`: internal team tool, domain contained a third-party trademark, anonymous visitors were redirected straight to a trademark-branded email+password form, plus a public waitlist form. Flagged as deceptive site; blocked in Brave/Chrome. Fix: deleted waitlist page, added neutral `/welcome` landing (ownership + non-affiliation notice), de-branded all logged-out pages, then Search Console review. Code was never the problem.
