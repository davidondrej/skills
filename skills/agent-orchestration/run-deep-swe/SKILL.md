...
The user has a dedicated OpenRouter key for this benchmark exported globally in their shell environment. A fresh shell already has `OPENROUTER_API_KEY` available. If it's somehow not set, re-source the shell environment:

```bash
source <shell-config-file> && echo "key loaded? ${OPENROUTER_API_KEY:+YES}"
```
