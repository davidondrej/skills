...
The user may have a dedicated OpenRouter key for this benchmark exported globally in `~/.zshrc` (weekly hard spend limit set as a safeguard). A fresh shell may already have `OPENROUTER_API_KEY` available. If it's somehow not set, re-source the shell:
